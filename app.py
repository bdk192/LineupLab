...
# a free-tier cloud instance. Update this periodically -- it does NOT
# advance on its own, so signals (especially contract-year, which uses
# max(SEASONS) as "today") will silently go stale if this isn't kept current.
SEASONS = [2025, 2026]
...
def load_participation(seasons):
    import nflreadpy as nfl
    # Sourced from FTN Data via nflverse; confirmed coverage through 2025
    # as of this writing. FTN provides this data AFTER each season ends,
    # so the current in-progress season (2026) will likely come back
    # empty/partial for this specific signal until the season is over --
    # the validation report will surface that rather than hide it.
    ...
    # nflreadpy raises ValueError (rather than returning empty) for a
    # season outside its supported range -- which happens routinely for
    # the current in-progress season, since FTN charting data isn't
    # published until after a season ends. Retry with progressively
    # older seasons dropped rather than crashing the whole app over it.
    seasons_to_try = sorted(seasons)
    df = None
    while seasons_to_try:
        try:
            df = nfl.load_participation(seasons_to_try)
            break
        except ValueError:
